<template>
  <div
    class="blueprint-node blueprint-node-if"
    ...
    <div class="blueprint-node-body">
      ...
      <div class="blueprint-block-info">
        ...
        <template v-for="(branch, i) in branches" :key="i">
          <div class="blueprint-block-info-line blueprint-block-info-code" :title="branch.condition">
            when {{ branch.conditionPreview }} → {{ branch.stepsCount }} step(s)
          </div>
        </template>
        <div v-if="elseStepsCount" class="blueprint-block-info-line blueprint-block-info-muted">
          else → {{ elseStepsCount }} step(s)
        </div>
      </div>
    </div>
  </div>
...
const props = defineProps<{
  ...
  showInput: boolean;
  sourceLabel: string;
  branches: { condition: string; conditionPreview: string; stepsCount: number }[];
  elseStepsCount: number;
